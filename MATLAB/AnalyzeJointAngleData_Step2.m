...
Peak_Knee_Flex = [max(squeeze(Avg_Joint_Angles_Walking(7,:,:))); ...
    max(squeeze(Avg_Joint_Angles_HeelWalking(7,:,:))); ...
    max(squeeze(Avg_Joint_Angles_ToeWalking(7,:,:))); ...
    max(squeeze(Avg_Joint_Angles_Ascend(7,:,:))); ...
    max(squeeze(Avg_Joint_Angles_Descend(7,:,:)))];
...
swarmchart([ones(1,50) 2*ones(1,50) 3*ones(1,50)],...
    [Peak_Ankle_Dorsi(1,:) Peak_Ankle_Dorsi(2,:) Peak_Ankle_Dorsi(3,:)],...
    'filled','MarkerFaceAlpha',0.5,'MarkerEdgeAlpha',0.5)

set(gca,'XTick',[1 2 3],...
    'XTickLabel',{'Walking' 'Heel Walking' 'Toe Walking'})
ylabel({'Peak Ankle'; 'Dorsiflexion (deg)'})
...
%--------------------------------------------------------------------------
% Create a table named 'Ankle_Dorsi_Table' organized as described in the
% comment just above this one. Be sure to add variable names 'Subject',
% 'Walking, 'Heel_Walking', and 'Toe_Walking' to the respective columns. 
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Create a swarm chart to illustrate the distribution of peak knee flexion
% angles for Walking, Heel Walking, and Toe Walking. Use filled markers
% with the MarkerFaceAlpha and MarkerEdgeAlpha properties each set to 0.5.
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Perform a repeated-measures ANOVA to determine if there are any
% statistically significant differences in peak knee flexion between
% Walking, Heel Walking, and Toe Walking.  
%--------------------------------------------------------------------------

%--------------------------------------------------------------------------
% Add annotations to the swarm chart to illustrate for which pairs of
% conditions the difference in peak knee flexion was found to be
